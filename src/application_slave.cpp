#include "application_slave.h"
#include "hdcp/exception.h"
#include "application.h"

using namespace hdcp;

ApplicationSlave::ApplicationSlave(common::Logger logger, Transport* transport, DataCallback data_cb,
...
void ApplicationSlave::send_command(Packet::BlockType id, const std::string& data, Request::Callback cb)
{
...
bool ApplicationSlave::wait_connected()
{
...
void ApplicationSlave::disconnect()
{
    disconnection_requested_ = true;
}

int ApplicationSlave::handler_state_disconnected_()
{
...
int ApplicationSlave::handler_state_connecting_()
{
//    if (statemachine_.get_nb_loop_in_current_state() == 1) {
...
int ApplicationSlave::handler_state_connected_()
{
    if (statemachine_.get_nb_loop_in_current_state() == 1) {
        dip_received_ = false;
...
void ApplicationSlave::wait_connection_request()
{
    std::unique_lock<std::mutex> lk(mutex_connection_);
